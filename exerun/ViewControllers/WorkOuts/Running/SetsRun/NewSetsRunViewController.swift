//
//  NewRunSetsViewController.swift
//  exerun
//
//  Created by Nazar Odemchuk on 25/10/2024.
//

import UIKit

class NewSetsRunViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    private func setupUI() {
        ...
        overlayView.backgroundColor = UIColor.black.withAlphaComponent(0.5) // Adjust alpha for desired darkness
        overlayView.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(overlayView)
        
        // Add constraints for the background image and overlay
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "StartSetsRunSegue", let setsRunViewController = segue.destination as? SetsRunViewController {
            setsRunViewController.timeTracker = QuickWorkoutModel(
    ...
    @objc private func startWorkoutButtonTapped() {
        ...
        if (totalWorkSeconds + totalRestSeconds) * totalSets < 60 {
            ...
            performSegue(withIdentifier: "StartSetsRunSegue", sender: self)
        }
    }
