...
class CyclingSessionModel {
    ...
    func updateHeartRate(_ heartRate: Int) {
        latestHeartRateValue = heartRate
    ...
    func updateLocation(location: CLLocation, lastLocation: CLLocation?) {
        ...
        let distance = last.distance(from: location) / 1000
    ...
    private func updateAveragePace() {
        // avgPace computed property handles it
    }
...
// MARK: – Cycling ----------------------------------------------------------
extension CyclingSessionModel: WorkoutSessionConvertible {
    typealias Entity = CyclingWorkOutEntity
    func fill(_ e: CyclingWorkOutEntity) {
        e.avgPace          = avgPace
