...
final class WeightUpdateView: UIView, UIPickerViewDataSource, UIPickerViewDelegate {
    private let picker = UIPickerView()
    private let label = UILabel()
    private var values = Array(30...200)
    private let onSelect: (Int) -> Void

    init(initialWeight: Int?, onSelect: @escaping (Int) -> Void) {
        self.onSelect = onSelect
        super.init(frame: .zero)
        setup()

        if let weight = initialWeight, let index = values.firstIndex(of: weight) {
